import checkFileExists from "../../../../services/Aws";
import s3bucket from "../../../../services/Aws";
...
async function saveObj(x: any, obj: any, total: any) {
    ...
    if (await checkFileExists(`${x.admno}.jpg`)) {
...
async function calulatePhotse(data: any) {
    const obj: any = {};
    const total: {
        total: number;
        photoleft: number;
    ...
    await Promise.all(data.map((x: any) => saveObj(x, obj, total)));
    // console.log(obj);
    obj["total"] = total;
    return obj;
}
...
export async function getTotalPhoto(req: any, res: any) {
    ...
    let total = 0;
    await Promise.all(classarr.map(async (x) => {
        ...
        classvalue[x] = await calulatePhotse(await sqlQueryp(query));
